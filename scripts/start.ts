...
async function main(): Promise<void> {
  ...
  if (skipTailscale) {
    publicHostname = process.env.WRANGLR_PUBLIC_HOST ?? "127.0.0.1";
    secure = process.env.WRANGLR_SECURE === "true";
    phoneUrl = `${secure ? "https" : "http"}://${publicHostname}:${pwaPort}`;
    console.log("Skipping Tailscale Serve (local-only mode).");
  } else {
  ...
  const serviceEnvironment = {
    ...process.env,
    WRANGLR_TOKEN: token,
    WRANGLR_BIND_HOST: process.env.WRANGLR_BIND_HOST ?? "127.0.0.1",
    ...
    WRANGLR_PWA_HOST: process.env.WRANGLR_PWA_HOST ?? "127.0.0.1",
